...
const officeFile = open('../resources/gotenberg/important/state_codes.xlsx')

export default function () {
    const url = apiUrl + 'public/api/v1/experiments/convert'
    const token = getAccessToken()

    const res = http.post(
        url,
        {
            files: http.file(officeFile, 'state_codes.xlsx')
        },
        {
            headers: {
